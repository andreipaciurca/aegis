...
func printAISetupPlan(plan ai.SetupPlan) {
	...
	if plan.Configured {
		fmt.Println()
		fmt.Println(colors.Green("Configured Aegis for local llama.cpp at " + ai.DefaultURL))
	}
	if plan.Run != nil && (plan.Run.Started || plan.Run.AlreadyRunning) {
		fmt.Println()
		fmt.Println(colors.Bold("Server"))
		if plan.Run.AlreadyRunning {
			fmt.Println(colors.Green("llama-server is already running"))
			return
		}
		fmt.Printf("pid:      %d\n", plan.Run.PID)
